...
- AI 评估会对每轮追问回答单独打分（`followUpAnswerScore`）。低于 70 分时提示“掌握不牢”，可点“加入题库”：选择题型（或让 AI 选择：概念辨析→单选/多选、术语→填空、原理→简答、C#/C++ 实现→算法题、设计任务→工程任务），AI 生成题干、选项/填空、参考实现、评分要点与解析；预览并调整标题、分类、难度、标签后保存。
