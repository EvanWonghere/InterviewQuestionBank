...
- The blog's music practice room (`/study/music/`) uses the same function through `music-*` actions handled by `music.ts`, with its own `music_messages` table. It explains, assigns homework and comments on scores; it never grades or marks mastery. See `docs/AI_TUTOR.md`.
...
`supabase/functions/ai-tutor/musicCatalog.json` is generated by the blog repository (`EvanWonghere/EvanWonghere.github.io`) with `node tools/export-music-catalog.mjs <path to this repository>` and must not be edited by hand. Regenerate it whenever lessons change there; the blog commits the matching `static/music/catalog-versions.mjs`. Music writes must not change quiz or lab records.
