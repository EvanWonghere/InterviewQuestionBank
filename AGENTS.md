...
- The stage game (`#/map`, `#/stage/:category/:index`) is a read-only layer over practice data. `gameRules.js` derives stages, stars, combo, XP and levels from questions, `reviewStates` and `attempts`; `gameStore.js` keeps only what cannot be derived (stage run records, best-star high-water marks, combo bonus XP, quiet mode) in local storage. It must not change grading, SM-2 scheduling or cloud tables. The third star needs a passing streak across two calendar days, and XP counts each question's best stars rather than attempts, because attempts keep only the latest 500. `AnswerPanel`'s `onRated(status, { quality, correct, assisted, aiScore })` feeds it. See `docs/GAMIFICATION.md`.
